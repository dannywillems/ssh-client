...
val with_shell_channel :
  session:ssh_session -> (shell_handle -> string -> 'a) -> 'a

val execute :
  ?read_stderr:bool -> ?read_timeout:int -> shell_handle -> string -> string

val scp :
  session:ssh_session -> src_path:string -> dst_path:string -> mode:int -> unit
