...
external connect_and_auth : options -> ssh_session -> unit
  = "libssh_ml_ssh_connect_and_auth"

external exec : command:string -> ssh_session -> string = "libssh_ml_ssh_exec"

external unsafe_scp : string -> string -> string -> int -> ssh_session -> unit
  = "libssh_ml_ssh_scp"
...
let with_shell f session =
  ...
  try
    ...
    result
  with
  | InternalError msg -> Channel.free chan ; failwith msg
  | exn -> Channel.free chan ; raise exn
